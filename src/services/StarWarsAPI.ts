...
export const getFilms = async (search: string, page: number) => {
  const data = await get<{ data: FilmInterface[]; total: number }>("films/", {
    search,
    page,
  });
...
export const getPeople = async (search: string, page: number) => {
  const data = await get<{ data: PeopleInterface[]; total: number }>(
    "people/",
    { search, page }
  );
  return { results: data.data, count: data.total };
...
export const getPlanets = async (search: string, page: number) => {
  const data = await get<{ data: PlanetInterface[]; total: number }>(
    "planets/",
    { search, page }
  );
  return { results: data.data, count: data.total };
...
export const getSpecies = async (search: string, page: number) => {
  const data = await get<{ data: SpeciesInterface[]; total: number }>(
    "species/",
    { search, page }
  );
  return { results: data.data, count: data.total };
...
export const getStarships = async (search: string, page: number) => {
  const data = await get<{ data: StarshipInterface[]; total: number }>(
    "starships/",
    { search, page }
  );
  return { results: data.data, count: data.total };
...
export const getVehicles = async (search: string, page: number) => {
  const data = await get<{ data: VehicleInterface[]; total: number }>(
    "vehicles/",
    { search, page }
  );
  return { results: data.data, count: data.total };
